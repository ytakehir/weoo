...
export const useHome = (user: User | null, missions: AreaMissionRow[] | null) => {
  const router = useRouter()
  const supabase = createClient()
  const [active, setActive] = useState(0)
  const [mission, setMission] = useState<AreaMissionRow | undefined>(missions?.[0])
  ...
  const judgeRegion = () => {
    const region = missions?.[active]?.region
    if (!region) return '地域'
